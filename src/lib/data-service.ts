...
const CURRENCIES = [
    { id: 'EUR', name: 'Euro', symbol: '6E', yield: 4.0, gdp: 0.8, cot: 15000, rsi: 45 },
    { id: 'GBP', name: 'British Pound', symbol: '6B', yield: 5.25, gdp: 0.5, cot: -5000, rsi: 55 },
    { id: 'JPY', name: 'Japanese Yen', symbol: '6J', yield: -0.1, gdp: 1.2, cot: -80000, rsi: 30 },
    { id: 'AUD', name: 'Australian Dollar', symbol: '6A', yield: 4.35, gdp: 1.5, cot: 2000, rsi: 60 },
    { id: 'CAD', name: 'Canadian Dollar', symbol: '6C', yield: 5.0, gdp: 1.1, cot: -1000, rsi: 48 },
    { id: 'CHF', name: 'Swiss Franc', symbol: '6S', yield: 1.75, gdp: 0.9, cot: 5000, rsi: 52 },
    { id: 'NZD', name: 'New Zealand Dollar', symbol: '6N', yield: 5.5, gdp: 1.8, cot: 1200, rsi: 58 },
    { id: 'MXN', name: 'Mexican Peso', symbol: '6M', yield: 11.25, gdp: 3.2, cot: 25000, rsi: 75 },
];
...
export function getFuturesData(): FuturePair[] {
    return CURRENCIES.map(c => {
        ...
        return {
            ...
            trendScore,
            factors,
            description: `${c.name} Futures Contract expiring Dec 2025`
        };
    }).sort((a, b) => b.trendScore - a.trendScore); // Sort by strongest trend
}

export function generateHistory(basePrice: number, days: number = 30) {
    const data = [];
    let price = basePrice;
    const now = new Date();
